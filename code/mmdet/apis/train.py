...
def train_detector(model,
    ...
    # use apex fp16 optimizer
    if cfg.swa_optimizer_config.get("type", None) and cfg.swa_optimizer_config["type"] == "DistOptimizerHook":
        if cfg.swa_optimizer_config.get("use_fp16", False):
            model, swa_optimizer = apex.amp.initialize(
                model.cuda(), swa_optimizer, opt_level="O1")
            for m in model.modules():
                if hasattr(m, "fp16_enabled"):
                    m.fp16_enabled = True
