...
def main():
    args = parse_args()
    args.cfg_list = ['ensemble_configs/cascade_mask_rcnn_swin_tiny.py',  # swin_tiny
                     'ensemble_configs/swa_cascade_mask_rcnn_swin_small_fpn.py',  # swa_swin_small
                     'ensemble_configs/swa_cascade_mask_rcnn_swin_base_fpn.py']   # swa_swin_base
    args.checkpoint = ['ensemble_configs/swin_tiny_e12-f4677e00.pth',
                       'ensemble_configs/swa_swin_small_e12-3369245f.pth',
                       'ensemble_configs/swa_swin_base-aea9dc1f.pth']
    cfg = mmcv.Config.fromfile(args.cfg_list[0])

    # 测试数据路径
    cfg.data.test['ann_file'] = '/data/user_data/testA.json'
    cfg.data.test['img_prefix'] = '/data/raw_data/test/images'

    # 修改配置
    cfg.data.test.pipeline[1]['img_scale'] = [(1333, 480), (1333, 576), (1333, 704), (1333, 800)]     
    ...
    for config_path, checkpoint_path in zip(args.cfg_list, args.checkpoint):
        ...
        models.append(model)
    model = EnsembleModel(models, cfg)  # 效果更好一些
    # model = EnsembleModelWBF(models, cfg)  # 采用wbf融合方式
