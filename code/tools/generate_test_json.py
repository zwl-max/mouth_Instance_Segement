...
label_ids = {name: i + 1 for i, name in enumerate(mouth_classes())}
...
def save(images, annotations):
    ...
    json.dump(ann, open('/data/user_data/testA.json', 'w'))
...
def test_dataset(im_dir):
    ...
    for im_path in tqdm(im_list):
        # image_id += 1
        image_id = int(os.path.basename(im_path).split('.')[0]) + 1
...
if __name__ == '__main__':
    test_dir = '/data/raw_data/test/images'
